...
@section('content')
    <main class="my-8">
        <section class="mx-auto max-w-7xl rounded-lg bg-[#294452] p-4 shadow-lg sm:p-6">
            <h1 class="fjalla text-4xl uppercase text-amber-400 sm:text-5xl">Overwatch Hero Counters Chart</h1>
            <p class="mt-4 max-w-4xl text-lg text-slate-100">Select your hero to see which opponents counter them, ranked from strongest to weakest counter. Use the role filters to narrow down results by Tank, Damage, or Support.</p>
            <div class="mt-4 rounded-md bg-[#1C2E37] p-4 text-sm text-slate-100">
                <strong>How the scoring system works:</strong> +20 means Hard Counter, +10 is favorable, 0 is neutral, -10 is unfavorable, and -20 means Weak Matchup.
            </div>
        </section>
        ...
        <section class="mx-auto mt-6 max-w-7xl rounded-lg bg-[#243b47] p-4 sm:p-6">
            ...
            <div class="mt-6 flex flex-wrap items-center justify-between gap-3">
                <h2 class="fjalla text-2xl uppercase text-white"><span id="selected-hero-label"></span> Counter Results</h2>
    ...
    <script>
        ...
        function renderResults() {
            ...
            const rows = heroes
                ...
                .map((hero) => ({ ...hero, role: heroRoles[hero.name] || 'Unknown', score: matrix[hero.name]?.[selectedHero] ?? 0 }))
